var App = function() {
    ...
    var toggleFunction = {
                            ...
                            if (document.querySelector('.main-container').classList.contains('sidebar-closed')) {
                                if (e === 'mouseenter') {
                                    ...
                                    if (activeSubmenu) {
                                        ...
                                        var activeMenuElement = document.querySelector('li.menu.active');
                                        var recentSubmenu = activeMenuElement ? activeMenuElement.querySelector('.collapse.submenu.recent-submenu') : null;
                                        if (recentSubmenu) {
                                            recentSubmenu.classList.add('show');
                                            var dropdownToggle = document.querySelector('.collapse.submenu.recent-submenu').parentNode.querySelector('.dropdown-toggle');
                                            if (dropdownToggle) {
        ...
        overlay: function() {
            var overlayElements = document.querySelectorAll('#dismiss, .overlay');
        ...
        search: function() {
            ...
            if (searchClose) {
                searchClose.addEventListener('click', function(event) {
                    ...
                    document.querySelector('body').classList.remove('search-active');
                    var searchFormControl = document.querySelector('.search-form-control');
            ...
            if (togglethemeEl) {
                togglethemeEl.addEventListener('click', function() {
                    try {
                        ...
                        if (parseObj && parseObj.settings && parseObj.settings.layout) {
                            if (parseObj.settings.layout.darkMode) {
                                ...
                                if (!getUpdatedParseObject.settings.layout.darkMode) {
                                    ...
                                    if (navbarLogo) {
                                        if (ifStarterKit) {
                                            navbarLogo.setAttribute('src', '{{ asset(\App\Models\Settings::first()->favicon) }}')
                                        } else if (getUpdatedParseObject.settings.layout.logo && getUpdatedParseObject.settings.layout.logo.lightLogo) {
                                ...
                                if (getUpdatedParseObject.settings.layout.darkMode) {
                                    ...
                                    if (navbarLogo) {
                                        if (ifStarterKit) {
                                            navbarLogo.setAttribute('src', '{{ asset(\App\Models\Settings::first()->favicon) }}')
                                        } else if (getUpdatedParseObject.settings.layout.logo && getUpdatedParseObject.settings.layout.logo.darkLogo) {
                        ...
                        // Fallback: just toggle dark class
