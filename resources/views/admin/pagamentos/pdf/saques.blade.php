...
@section('title', 'Relatório de Saques')

@section('filtro-label', 'Filtro de Usuário')

@section('total-registros', count($saques))

@section('total-title', 'Total de Saques Concluídos')
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                {{ $saque->usuario_nome }}
                @if($saque->isaf == 2)
                    <small style="color: #28a745; font-weight: bold;">(Bônus)</small>
                @endif
            </td>
            <td>{{ $saque->cpf }}</td>
...
<div style="margin-top: 20px; font-size: 11px; color: #666;">
    ...
    <table style="width: 100%; margin-top: 10px;">
        <tr>
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">{{ count($saques) }}</td>
        </tr>
        <tr>
            <td style="background-color: #f8f9fa; padding: 8px; border: 1px solid #ddd; font-weight: bold;">Saques Concluídos:</td>
            <td style="padding: 8px; border: 1px solid #ddd;">{{ $saques->where('status', 1)->count() }}</td>
        </tr>
        <tr>
            <td style="background-color: #f8f9fa; padding: 8px; border: 1px solid #ddd; font-weight: bold;">Saques Pendentes:</td>
            <td style="padding: 8px; border: 1px solid #ddd;">{{ $saques->where('status', 0)->count() }}</td>
        </tr>
        <tr>
            <td style="background-color: #f8f9fa; padding: 8px; border: 1px solid #ddd; font-weight: bold;">Saques Cancelados:</td>
            <td style="padding: 8px; border: 1px solid #ddd;">{{ $saques->where('status', 2)->count() }}</td>
        </tr>
        <tr>
            <td style="background-color: #f8f9fa; padding: 8px; border: 1px solid #ddd; font-weight: bold;">Saques Bônus:</td>
            <td style="padding: 8px; border: 1px solid #ddd;">{{ $saques->where('isaf', 2)->count() }}</td>
        </tr>
        <tr>
            <td style="background-color: #f8f9fa; padding: 8px; border: 1px solid #ddd; font-weight: bold;">Valor Total (Todos):</td>
            <td style="padding: 8px; border: 1px solid #ddd;">R$ {{ number_format($saques->sum('amount'), 2, ',', '.') }}</td>
        </tr>
        <tr>
