...
@section('title', 'Editar Produto - ' . $raspadinha->name)
        ...
        <div class="page-meta" style="margin-top:45px;">
            <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page" id="estatde">Editar Produto</li>
                </ol>
            </nav>
        </div>
                ...
                <div class="widget-content widget-content-area br-8" style="padding:20px;">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-12 d-flex justify-content-between align-items-center">
                            <div>
                                <h4 class="m-0">Editar Produto: {{ $item->name }}</h4>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="text-center">
                                        <h4 class="mb-0" id="total-probability">{{ number_format($raspadinha->items()->where('id', '!=', $item->id)->sum('probability'), 2) }}%</h4>
                                        <small class="text-muted">Já Configurado (outros produtos)</small>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="text-center">
                                        <h4 class="mb-0 text-success" id="remaining-probability">{{ number_format(100 - $raspadinha->items()->where('id', '!=', $item->id)->sum('probability'), 2) }}%</h4>
                                        <small class="text-muted">Disponível (incluindo este produto)</small>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="text-center">
                    ...
                    <form action="{{ route('admin.raspadinha-item.update', [$raspadinha, $item]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control @error('premio_type') is-invalid @enderror" id="premio_type" 
                                        ...
                                        @foreach(\App\Models\RaspadinhaItem::PREMIO_TYPES as $key => $label)
                                            <option value="{{ $key }}" {{ old('premio_type', $item->premio_type ?? '') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('premio_type')
                                        <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6" id="value-field">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" class="form-control @error('value') is-invalid @enderror" id="value" 
                                           name="value" value="{{ old('value', $item->value) }}" step="0.01" min="0" required>
                            ...
                            <div class="col-md-6" id="product-description-field" style="display: none;">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control @error('product_description') is-invalid @enderror" id="product_description" 
                                              name="product_description" rows="3" placeholder="Ex: iPhone 15 Pro Max 256GB">{{ old('product_description', $item->product_description ?? '') }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" class="form-control @error('probability') is-invalid @enderror" id="probability" 
                                           name="probability" value="{{ old('probability', $item->probability) }}" step="0.01" min="0.01" 
                                           max="{{ 100 - $raspadinha->items()->where('id', '!=', $item->id)->sum('probability') }}" required>
                                    <small class="text-muted">Máximo: {{ number_format(100 - $raspadinha->items()->where('id', '!=', $item->id)->sum('probability'), 2) }}%</small>
                                    @error('probability')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    @if($item->image)
                                        <div class="mb-2">
                                            <img src="{{ $item->image_url }}" alt="{{ $item->name }}" height="60" class="mb-2">
                        ...
                        <div class="form-group mb-3">
                            <button type="submit" class="btn btn-primary" id="update-button">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-save"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> Atualizar Produto
                            </button>
                        </div>
                    </form>
...
<script>
    $(function() {
        ...
        const maxProbability = {{ 100 - $raspadinha->items()->where('id', '!=', $item->id)->sum('probability') }};
