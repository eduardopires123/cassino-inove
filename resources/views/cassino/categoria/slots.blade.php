    ...
    <section data-v-0a4c896b="" class="spacing"  id="content-game">
        <header class="qMh00">
            <div class="Efcyy">
                ...
                <div class="f22UR">{{ __('slots.all_casino_slots') }}</div>
                <div class="hJNny">
            ...
            <div data-v-959816fa="" class="casino-filters src7V">
                <div data-v-959816fa="" class="casino-search" style="padding: 4px;">
                    <input data-v-959816fa="" class="_6z1L2" id="search-input" placeholder="{{ __('slots.search_game') }}" autocomplete="nope" onfocus="this.removeAttribute('readonly');" readonly aria-autocomplete="none">
                </div>
                <div data-v-959816fa="" class="casino-filters__buttons grid grid-cols-2 gap-4">
                    <div data-v-959816fa="" class="listBox-wrapper">
                        <button data-v-959816fa="" id="headlessui-listbox-button-nsiNM9WAguS_1" type="button" aria-haspopup="listbox" aria-expanded="false" data-headlessui-state="" class="select-btn provider-select-btn">
                            {{ __('slots.providers') }}: <strong data-v-959816fa="" id="selected-provider">{{ __('slots.all') }}</strong>
                        </button>
                        <ul data-v-959816fa="" id="headlessui-listbox-options-nsiNM9WAguS_2" aria-multiselectable="true" aria-labelledby="headlessui-listbox-button-nsiNM9WAguS_1" aria-orientation="vertical" role="listbox" tabindex="0" data-headlessui-state="" class="select-options no-scrollbar provider-options" style="display: none;">
                            <li data-v-959816fa="" id="headlessui-listbox-option-all" role="option" tabindex="-1" aria-selected="true" data-headlessui-state="" class="select-opt">
                                <label data-v-959816fa="" for="all-providers"><input data-v-959816fa="" id="all-providers" name="opt" type="checkbox" value="all" checked /><span data-v-959816fa="" class="ml-2">{{ __('slots.all') }}</span></label>
                            </li>
                            <!-- Os provedores serão inseridos aqui via JavaScript -->
                        </ul>
                    </div>
                    <div data-v-959816fa="" class="listBox-wrapper">
                        <button data-v-959816fa="" id="headlessui-listbox-button-nsiNM9WAguS_3" type="button" aria-haspopup="listbox" aria-expanded="false" data-headlessui-state="" class="select-btn" disabled>
                            {{ __('slots.category') }}: <strong data-v-959816fa="">{{ __('slots.slots') }}</strong>
                        </button>
                        <!---->
                    </div>
                </div>
        ...
        <div data-v-14a18591="" data-v-0a4c896b="" class="pagination">
            ...
            <span data-v-14a18591="" class="text-texts text-xs mb-4" id="showing-text">{{ __('slots.showing') }} {{ $current }} {{ __('slots.of') }} {{ $total }} {{ __('slots.games') }}</span>
            <button data-v-14a18591="" class="btn-more" id="load-more-btn" data-page="2"><span data-v-14a18591="">{{ __('slots.load_more') }}</span></button>
        </div>
    </section>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadProviders() {
                // Modificar a URL para incluir o filtro de categoria 'slots'
                fetch('/provedores/listar?category=slots')
                    .then(response => {
                        if (!response.ok) {
                    ...
                    .then(data => {
                        ...
                        fetch('/provedores/verificar-ativos', {
                            ...
                            body: JSON.stringify({
                                ...
                                category: 'slots' // Adicionar categoria ao verificar ativos
                            })
                        })
                            .then(response => {
                                if (!response.ok) {
                            ...
                            .then(activeData => {
                                ...
                                if (!activeData.active_providers) {
                                    ...
                                    selectedProviderText.textContent = '{{ __("slots.data_error") }}';
                                    return;
                                }
                                ...
                                if (activeProviders.length === 0) {
                                    selectedProviderText.textContent = '{{ __("slots.none_active") }}';
                                    return;
                                }
                        ...
                        `;
                                ...
                                selectedProviderText.textContent = '{{ __("slots.all") }}';
                                    ...
                                    checkbox.addEventListener('change', function() {
                                        if (this.value === 'all') {
                                            ...
                                            if (checkedProviders.length === 1) {
                                                ...
                                                selectedProviderText.textContent = `${checkedProviders.length} {{ __("slots.selected") }}`;
                                            } else {
                            ...
                            .catch(error => {
                                ...
                                selectedProviderText.textContent = '{{ __("slots.error") }}';
                    ...
                    .catch(error => {
                        ...
                        selectedProviderText.textContent = '{{ __("slots.unavailable") }}';
                    });
            }
            ...
            // Função para resetar e carregar jogos
            function resetAndLoadGames() {
                ...
                loadMoreBtn.innerHTML = '<span>{{ __("slots.load_more_games") }}</span>';
            ...
            function setupSearchFunctionality() {
                ...
                function processSearch() {
                    ...
                    loadMoreBtn.style.display = 'block';
                    loadMoreBtn.disabled = false;
                    loadMoreBtn.innerHTML = '<span>{{ __("slots.load_more") }}</span>';
            ...
            // Função para carregar mais jogos (atualizada para slots)
            loadMoreBtn.addEventListener('click', function() {
                const page = parseInt(this.getAttribute('data-page'));

                // Adicionar indicador de carregamento
                loadMoreBtn.disabled = true;
                loadMoreBtn.innerHTML = '<span>{{ __("slots.loading") }}</span>';

                // Atualizar URL para usar categoria slots
                let url = `/jogos/carregar-mais?page=${page}&per_page=24&category=slots`;
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        <div class="alert alert-warning"><span>{{ __("slots.no_results") }}</span></div>
                    `;
                        ...
                        showingText.textContent = `{{ __("slots.showing") }} ${currentShowing} {{ __("slots.of") }} ${teste} {{ __("slots.games") }}`;
