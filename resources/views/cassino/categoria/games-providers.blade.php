    ...
    <section data-v-0a4c896b="" class="spacing"  id="content-game">
        <header class="qMh00">
            <div class="Efcyy">
                ...
                <div id="jogos_de" class="f22UR">{{ __('casino-provider.all_casino_provedor') }}</div>
                <div class="hJNny">
            ...
            <div data-v-959816fa="" class="casino-filters src7V">
                <div data-v-959816fa="" class="casino-search" style="padding: 4px;">
                    <input data-v-959816fa="" class="_6z1L2" id="search-input" placeholder="{{ __('casino-provider.search_game') }}" autocomplete="nope" onfocus="this.removeAttribute('readonly');" readonly aria-autocomplete="none">
                </div>
                <div data-v-959816fa="" class="casino-filters__buttons grid grid-cols-2 gap-4">
                    <div data-v-959816fa="" class="listBox-wrapper">
                        <button data-v-959816fa="" id="headlessui-listbox-button-nsiNM9WAguS_1" type="button" aria-haspopup="listbox" aria-expanded="false" data-headlessui-state="" class="select-btn provider-select-btn">
                            {{ __('casino-provider.providers') }}: <strong data-v-959816fa="" id="selected-provider">{{ __('casino-provider.all') }}</strong>
                        </button>
                        <ul data-v-959816fa="" id="headlessui-listbox-options-nsiNM9WAguS_2" aria-multiselectable="true" aria-labelledby="headlessui-listbox-button-nsiNM9WAguS_1" aria-orientation="vertical" role="listbox" tabindex="0" data-headlessui-state="" class="select-options no-scrollbar provider-options" style="display: none;">
                            <li data-v-959816fa="" id="headlessui-listbox-option-all" role="option" tabindex="-1" aria-selected="true" data-headlessui-state="" class="select-opt">
                                <label data-v-959816fa="" for="all-providers"><input data-v-959816fa="" id="all-providers" name="opt" type="checkbox" value="all" checked /><span data-v-959816fa="" class="ml-2">{{ __('casino-provider.all') }}</span></label>
                            </li>
                            <!-- Os provedores serão inseridos aqui via JavaScript -->
                        </ul>
                    </div>
                    <div data-v-959816fa="" class="listBox-wrapper">
                        <button data-v-959816fa="" id="headlessui-listbox-button-nsiNM9WAguS_3" type="button" aria-haspopup="listbox" aria-expanded="false" data-headlessui-state="" class="select-btn category-select-btn">
                            {{ __('casino-provider.categories') }}: <strong data-v-959816fa="" id="selected-category">{{ __('casino-provider.all') }}</strong>
                        </button>
                        <ul data-v-959816fa="" id="headlessui-listbox-options-nsiNM9WAguS_4" aria-multiselectable="true" aria-labelledby="headlessui-listbox-button-nsiNM9WAguS_3" aria-orientation="vertical" role="listbox" tabindex="0" data-headlessui-state="" class="select-options no-scrollbar category-options" style="display: none;">
                            <li data-v-959816fa="" id="headlessui-listbox-option-all-category" role="option" tabindex="-1" aria-selected="true" data-headlessui-state="" class="select-opt">
                                <label data-v-959816fa="" for="all-categories"><input data-v-959816fa="" id="all-categories" name="category-opt" type="checkbox" value="all" checked /><span data-v-959816fa="" class="ml-2">{{ __('casino-provider.all') }}</span></label>
                            </li>
        ...
        <div data-v-14a18591="" data-v-0a4c896b="" class="pagination">
            <div data-v-14a18591="" class="w-[40%] lg:w-[16%] bg-texts/10 rounded-md h-1 mb-4">
                <div data-v-14a18591="" class="bg-primary h-1 rounded-md" id="progress-bar" style="width: {{ ($current ?? 1 / $total ?? 1) * 100 }}%;"></div>
            </div>
            <span data-v-14a18591="" class="text-texts text-xs mb-4" id="showing-text">{{ __('casino-provider.showing') }} {{ $current }} {{ __('casino-provider.of') }} {{ $total }} {{ __('casino-provider.games') }}</span>
            <button data-v-14a18591="" class="btn-more" id="load-more-btn" data-page="2"><span data-v-14a18591="">{{ __('casino-provider.load_more') }}</span></button>
        </div>
    </section>

    <script>
        // Script para gerenciamento de jogos de cassino
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Função para verificar se há um provedor na URL
            function checkProviderInUrl() {
                const urlPath = window.location.pathname;
                const pathParts = urlPath.split('/');

                // Verifica se a URL contém o padrão "/cassino/provider/NOMEPROVEDOR"
                if (pathParts.length >= 4 && pathParts[1] === 'cassino' && pathParts[2] === 'provider') {
                    const providerSlug = pathParts[3];
                    return providerSlug;
                }
                return null;
            }

            // Função para selecionar um provedor com base no slug da URL
            function selectProviderBySlug(slug) {
                // Esperar até que os provedores sejam carregados
                const checkProvidersLoaded = setInterval(() => {
                    const providerCheckboxes = document.querySelectorAll('.provider-options input[type="checkbox"]');

                    if (providerCheckboxes.length > 1) { // Garantir que os provedores foram carregados
                        clearInterval(checkProvidersLoaded);

                        // Normalizar o slug para comparação
                        const normalizedSlug = slug.toLowerCase().replace(/[^a-z0-9]/g, '');

                        // Procurar o provedor correspondente
                        let providerFound = false;
                        providerCheckboxes.forEach(checkbox => {
                            if (checkbox.value !== 'all') {
                                const checkboxSlug = checkbox.value.toLowerCase().replace(/[^a-z0-9]/g, '');

                                if (checkboxSlug === normalizedSlug) {
                                    // Desmarcar "Todos"
                                    document.getElementById('all-providers').checked = false;

                                    // Marcar o provedor encontrado
                                    checkbox.checked = true;
                                    selectedProvider = checkbox.value;
                                    selectedProviderText.textContent = selectedProvider;
                                    providerFound = true;

                                    document.getElementById("jogos_de").textContent = "{{ __('casino-provider.all_casino_provedor') }} " + selectedProvider;
                ...
                fetch('/provedores/listar')
                    ...
                    .then(data => {
                        ...
                        fetch('/provedores/verificar-ativos', {
                            ...
                            .then(activeData => {
                                ...
                                if (!activeData.active_providers) {
                                    ...
                                    selectedProviderText.textContent = '{{ __("casino-provider.data_error") }}';
                                    return;
                                }
                                ...
                                if (activeProviders.length === 0) {
                                    selectedProviderText.textContent = '{{ __("casino-provider.none_active") }}';
                                    return;
                                }
                        ...
                        `;
                                ...
                                selectedProviderText.textContent = '{{ __("casino-provider.all") }}';

                                // Verificar se há um provedor na URL
                                const urlProvider = checkProviderInUrl();
                                if (urlProvider) {
                                    // Tentar selecionar o provedor da URL
                                    selectProviderBySlug(urlProvider);
                                }
                                    ...
                                    checkbox.addEventListener('change', function() {
                                        if (this.value === 'all') {
                                            ...
                                            if (checkedProviders.length === 1) {
                                                ...
                                                selectedProviderText.textContent = `${checkedProviders.length} {{ __("casino-provider.selected") }}`;
                                            } else {
                            ...
                            .catch(error => {
                                ...
                                selectedProviderText.textContent = '{{ __("casino-provider.error") }}';
                    ...
                    .catch(error => {
                        ...
                        selectedProviderText.textContent = '{{ __("casino-provider.unavailable") }}';
                    });
            }
            ...
            function loadCategories() {
                fetch('/categorias/listar')
                        ...
                        categoryCheckboxes.forEach(checkbox => {
                            checkbox.addEventListener('change', function() {
                                if (this.value === 'all') {
                                    ...
                                    selectedCategoryText.textContent = '{{ __("casino-provider.all") }}';
                                } else {
                                    ...
                                    if (checkedCategories.length === 1) {
                                        ...
                                        selectedCategoryText.textContent = `${checkedCategories.length} {{ __("casino-provider.multiple_selected") }}`;
                                    } else {
                    ...
                    .catch(error => {
                        ...
                        // Exibir mensagem de erro ao usuário
                        selectedCategoryText.textContent = '{{ __("casino-provider.unavailable") }}';
                    });
            }
            ...
            function setupSearchFunctionality() {
                ...
                function processSearch() {
                    ...
                    loadMoreBtn.innerHTML = '<span>{{ __("casino-provider.load_more") }}</span>';
            ...
            loadMoreBtn.addEventListener('click', function() {
                ...
                loadMoreBtn.innerHTML = '<span>{{ __("casino-provider.loading") }}</span>';
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        <div class="alert alert-warning"><span>{{ __("casino-provider.no_results") }}</span></div>
                    `;
                                ...
                                <div class="u3Qxq">
                                    ...
                                    <section class="bBtlK">
                                        <span class="Oe7Pi">
                                            ...
                                            <span>{{ __("casino-provider.play") }}</span>
                                        </span>
                                    </section>
                                </div>
                            </a>
                        ...
                        showingText.textContent = `{{ __("casino-provider.showing") }} ${currentShowing} {{ __("casino-provider.of") }} ${teste} {{ __("casino-provider.games") }}`;
                        ...
                        if (currentShowing >= data.total) {
                            ...
                            loadMoreBtn.style.display = 'block';
                            loadMoreBtn.disabled = false;
                            loadMoreBtn.innerHTML = '<span>{{ __("casino-provider.load_more_games") }}</span>';
                        }
                    })
                    .catch(error => {
                        ...
                        loadMoreBtn.disabled = false;
                        loadMoreBtn.innerHTML = '<span>{{ __("casino-provider.try_again") }}</span>';

                        // Mostrar mensagem de erro ao usuário
                        if (!gamesContainer.querySelector('.error-message')) {
                            const errorElement = `
                        <div class="error-message p-4 text-center">
                            <p>{{ __("casino-provider.error_loading") }}</p>
                        </div>
                    `;
